...
get_symm_buffer_size_for_sm90_fp8_mega_moe(
    ...
    //   * No shared-expert support yet: the four shared slots are empty tensors.
    DG_HOST_ASSERT(num_experts % num_ranks == 0);
    ...
    // Combine buffer: BF16 tokens, one landing slot per topk
    ...
    auto slice_input_buffers = [=](const torch::Tensor& buffer) {
        ...
        auto l2_acts_sf = torch::from_blob(
            ...
            {1, num_max_padded_sf_pool_tokens},
            torch::TensorOptions().dtype(torch::kFloat32).device(buffer.device()));
        // No shared-expert support on SM90 yet: shared slots are empty views
        return std::make_tuple(x, x_sf, topk_idx, topk_weights,
...
// float scale factors, and no shared experts. Ported from PR #360.
static void fp8_mega_moe(
    ...
    // Already registered tensors (shared slots unused on SM90)
    const auto [x, x_sf, topk_idx, topk_weights,
                _shared_l1_acts, _shared_l1_acts_sf, _shared_l2_acts, _shared_l2_acts_sf,
                l1_acts, l1_acts_sf, l2_acts, l2_acts_sf] = slice(sym_buffer);
